...
class Pond(Base):
    ...
    location:Mapped[object|None]=mapped_column(Text().with_variant(Geometry(geometry_type="POINT", srid=4326, spatial_index=False), "postgresql"), nullable=True)
...
class PondGeometry(Base):
    ...
    geom:Mapped[object|None]=mapped_column(Text().with_variant(Geometry(geometry_type='GEOMETRY', srid=4326, spatial_index=False), 'postgresql'), nullable=True)
