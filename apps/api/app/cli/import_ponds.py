...
def main():
    ...
    p.add_argument('--batch-size', type=int, default=5000); p.add_argument('--limit', type=int); p.add_argument('--dry-run', action='store_true'); p.add_argument('--report')
    args=p.parse_args()
    with SessionLocal() as db: stats=import_excel(db,args.input,limit=args.limit,mode='dry-run' if args.dry_run else args.mode,batch_size=args.batch_size)
