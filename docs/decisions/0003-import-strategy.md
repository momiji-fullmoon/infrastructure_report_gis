...
COPY + staging はさらに高速化できるが、MVPでは品質フラグ、失敗行サンプル、SQLite単体テスト互換、実装リスクのバランスから Core バッチUPSERTを採用した。冪等性は `(source_system, source_record_id)` 一意制約とデフォルト `upsert` で担保する。`--resume` は先頭から安全に再実行できるUPSERT方式として扱い、中断後に重複を増やさない。
